...
=======================
MetaHuman right-eye shader → Arnold aiStandardSurface converter.

Source: dx11_shd_eyeRight.fx (MetaHumanForMaya plugin v1.3.x).
...
USAGE
-----
    import mh_eyeRight_to_arnold as eye_l
    eye_l.convert()
...
DX11_SHADER   = "shader_eyeRight_shader"
ARNOLD_SHADER = "eyeRight_aiStandardSurface"
...
def _resolve_dx11_shader():
    ...
    for name in candidates:
        if cmds.objExists(name):
            if name != DX11_SHADER:
                print("[mh2arnold-eyeR] Resolved DX11 shader '{}' (was '{}')".format(name, DX11_SHADER))
                DX11_SHADER = name
            return name
    return None
...
def _set_color_space(node, space):
    ...
    try:
        ...
        cmds.warning("[mh2arnold-eyeR] colorSpace fail on {}: {}".format(node, e))
...
def _autodetect_textures(dx11_shader):
    ...
    if not cmds.objExists(dx11_shader):
        cmds.error("[mh2arnold-eyeR] '{}' not found.".format(dx11_shader))
        return {}

    # DX11 shader attribute names (defined in dx11_shd_eyeRight.fx)
    attrs = {
...
def _cleanup_previous_run():
    for n in (ARNOLD_SHADER, "eyeRight_aiNormalMap", "eyeRight_specCavityMul",
              "eyeRight_DEBUG_surfaceShader"):
        if cmds.objExists(n):
            try:
                cmds.delete(n)
            except Exception:
...
def convert():
    ...
    if not _resolve_dx11_shader():
        cmds.error("[mh2arnold-eyeR] DX11 shader not found (tried multiple variants).")
        return

    print("[mh2arnold-eyeR] === STARTING CONVERSION (eyeRight) ===")
    _cleanup_previous_run()

    tex = _autodetect_textures(DX11_SHADER)
    print("[mh2arnold-eyeR] Auto-detected textures:")
    ...
    if _has_valid_path(tex["normal"]):
        nmap = _create_node("aiNormalMap", "eyeRight_aiNormalMap")
    ...
    if _has_valid_path(tex["cavity"]):
        mul = _create_node("multiplyDivide", "eyeRight_specCavityMul")
    ...
    if sg:
        ...
        print("[mh2arnold-eyeR] SG '{}' surfaceShader -> {}".format(sg, ai))
    else:
        cmds.warning("[mh2arnold-eyeR] No shadingEngine found for {}".format(DX11_SHADER))

    print("[mh2arnold-eyeR] === DONE ===")
    return ai


def revert_to_dx11():
    if not _resolve_dx11_shader():
        cmds.error("[mh2arnold-eyeR] DX11 shader not found.")
        return
    sg = _get_shadingengine(DX11_SHADER) or _get_shadingengine(ARNOLD_SHADER)
    if not sg:
        cmds.error("[mh2arnold-eyeR] No shadingEngine to restore.")
        return
    _connect(DX11_SHADER + ".outColor", sg + ".surfaceShader")
    print("[mh2arnold-eyeR] Reverted: {} -> {}".format(DX11_SHADER, sg))
