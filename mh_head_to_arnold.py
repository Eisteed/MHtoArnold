...
====================
MetaHuman head shader → Arnold aiStandardSurface converter.

Designed for the new MetaHuman for Maya plugin (UE 5.6+, plugin v1.3.x),
which uses a single dx11Shader node named `shader_head_shader` with:
...
DX11_SHADER = "shader_head_shader"
ARNOLD_SHADER = "head_aiStandardSurface"

# Texture file node names produced by MetaHumanForMaya plugin v1.3.x
TEX = {
    "diffuse":   "baseMapFile_head_color",
    "normal":    "baseMapFile_head_normal",
    "cavity":    "baseMapFile_head_cavity",
    "cm": {
        1: "mapFile_head_cm1_color",
        2: "mapFile_head_cm2_color",
        3: "mapFile_head_cm3_color",
    },
    "wm": {
        1: "mapFile_head_wm1_normal",
        2: "mapFile_head_wm2_normal",
        3: "mapFile_head_wm3_normal",
    },
    "masks": [
        "maskFile_head_wm1_01", "maskFile_head_wm1_02",
        "maskFile_head_wm1_03", "maskFile_head_wm1_04",
        "maskFile_head_wm2_01", "maskFile_head_wm2_02", "maskFile_head_wm2_03",
        "maskFile_head_wm3_01", "maskFile_head_wm3_02",
        "maskFile_head_wm13_01",
    ],
}
...
def _cleanup_previous_run():
    ...
    stale_names = [
        "head_aiNormalMap",
        "head_specCavityMul",
        "head_sssColorMix",      # legacy aiMix attempt — may exist as unknown
        "head_sssColorBlend",
        "head_DEBUG_surfaceShader",   # debug shader from viewport_debug_on()
        ARNOLD_SHADER,
    ]
    for n in stale_names:
        if cmds.objExists(n):
            try:
                ...
                cmds.warning("[mh2arnold] Could not delete stale node '{}': {}".format(n, e))

    # v2 wrinkle compositor nodes — delete by name prefix
    # These are generated programmatically (~135 nodes), so wildcard cleanup.
    v2_prefixes = [
        "head_lerp_sub_",                         # 6 lerp-delta subtracts (cm/wm)
        "head_delta_sub_", "head_delta_mul_",     # legacy (pre-fix) names — remove if present
        "head_colC_dxm_", "head_colC_w_",         # color contribs
        "head_nrmC_dxm_", "head_nrmC_w_",         # normal contribs
        "head_sum_", "head_final_",               # accumulators
    ...
    for u in unknown_nodes:
        if u.startswith("head_"):
            try:
                cmds.lockNode(u, lock=False)
                cmds.delete(u)
                print("[mh2arnold] Removed unknown stale node: {}".format(u))
            except Exception:
                pass
...
def _get_shadingengine(shader):
    ...
    for mesh_candidate in ("head_lod0_mesh", "head_lod0_meshShape",
                           "head_lod0_mesh_skinning_geometry",
                           "m_med_nrw_head_lod0_meshShape"):
...
def _build_base_color(ai_shader):
    ...
    diffuse = TEX["diffuse"]
    if not cmds.objExists(diffuse):
        cmds.warning("[mh2arnold] Missing diffuse file node: {}".format(diffuse))
        return
...
def _build_normal(ai_shader):
    ...
    normal = TEX["normal"]
    if not cmds.objExists(normal):
        cmds.warning("[mh2arnold] Missing normal file node: {}".format(normal))
        return

    nmap = _create_node("aiNormalMap", "head_aiNormalMap")
...
def _silence_empty_file_node(file_node):
    ...
    if path.endswith("/") or path.endswith("\\"):
        try:
            ...
            print("[mh2arnold] Cleared empty path on '{}' to silence warnings.".format(file_node))
        except Exception as e:
            cmds.warning("[mh2arnold] Could not clear path on {}: {}".format(file_node, e))
...
def _build_cavity_into_specular(ai_shader):
    ...
    if not _is_valid_texture_path(cavity):
        cmds.warning("[mh2arnold] Cavity unavailable — using flat specular.")
        _silence_empty_file_node(cavity)
        cmds.setAttr(ai_shader + ".specular", SKIN_SPEC["specular"])
        return

    # spec_weight = baseSpec * cavity_R
    mul = _create_node("multiplyDivide", "head_specCavityMul")
...
def _set_sss_and_spec(ai_shader):
    ...
    diffuse = TEX["diffuse"]
    if cmds.objExists(diffuse):
        blend = _create_node("blendColors", "head_sssColorBlend")
...
# channel_idx → (mask_file_node_name, RGBA component letter)
# Each maskChannel_NN samples exactly ONE component of one texture file.
# The component cycles R→G→B→A within each file group, then resets.
HLSL_CHANNEL_MAP = {
    # wm1 file group: 4 files, 15 channels (4+4+4+3)
     0: ("maskFile_head_wm1_01", "R"),  1: ("maskFile_head_wm1_01", "G"),
     2: ("maskFile_head_wm1_01", "B"),  3: ("maskFile_head_wm1_01", "A"),
     4: ("maskFile_head_wm1_02", "R"),  5: ("maskFile_head_wm1_02", "G"),
     6: ("maskFile_head_wm1_02", "B"),  7: ("maskFile_head_wm1_02", "A"),
     8: ("maskFile_head_wm1_03", "R"),  9: ("maskFile_head_wm1_03", "G"),
    10: ("maskFile_head_wm1_03", "B"), 11: ("maskFile_head_wm1_03", "A"),
    12: ("maskFile_head_wm1_04", "R"), 13: ("maskFile_head_wm1_04", "G"),
    14: ("maskFile_head_wm1_04", "B"),
    # wm2 file group: 3 files, 10 channels (4+4+2)
    15: ("maskFile_head_wm2_01", "R"), 16: ("maskFile_head_wm2_01", "G"),
    17: ("maskFile_head_wm2_01", "B"), 18: ("maskFile_head_wm2_01", "A"),
    19: ("maskFile_head_wm2_02", "R"), 20: ("maskFile_head_wm2_02", "G"),
    21: ("maskFile_head_wm2_02", "B"), 22: ("maskFile_head_wm2_02", "A"),
    23: ("maskFile_head_wm2_03", "R"), 24: ("maskFile_head_wm2_03", "G"),
    # wm3 file group: 2 files, 8 channels (4+4)
    25: ("maskFile_head_wm3_01", "R"), 26: ("maskFile_head_wm3_01", "G"),
    27: ("maskFile_head_wm3_01", "B"), 28: ("maskFile_head_wm3_01", "A"),
    29: ("maskFile_head_wm3_02", "R"), 30: ("maskFile_head_wm3_02", "G"),
    31: ("maskFile_head_wm3_02", "B"), 32: ("maskFile_head_wm3_02", "A"),
    # wm13 file group (shared between cm1/wm1 and cm3/wm3): 1 file, 4 channels
    33: ("maskFile_head_wm13_01", "R"), 34: ("maskFile_head_wm13_01", "G"),
    35: ("maskFile_head_wm13_01", "B"), 36: ("maskFile_head_wm13_01", "A"),
}
...
def _parse_weight_drivers():
    ...
    for i in range(100):  # enough headroom; actual count is 82
        ...
        if "_color_" in attr:
            ...
            cmds.warning("[mh2arnold] Unparsable weight driver: " + attr)
            continue
        ...
        for n in (1, 2, 3):
            ...
            if attr.startswith(prefix):
                ...
                break
        if map_idx is None:
            cmds.warning("[mh2arnold] Could not extract map_idx: " + attr)
            continue
        ...
        for g in _GROUP_ORDER:
            ...
            if rest.startswith(tag):
                ...
                break
        if group is None:
            cmds.warning("[mh2arnold] Could not extract group: " + attr)
            continue
...
def _build_zone_to_channel_mapping(weights):
    ...
    for group in _GROUP_ORDER:
        ...
        for i, zone in enumerate(zones):
            ...
            if file_idx >= len(files):
                cmds.warning("[mh2arnold] Group {} has more zones ({}) than "
    ...
    for key, val in ZONE_MAPPING_OVERRIDE.items():
        if key in mapping:
            ...
            cmds.warning("[mh2arnold] Override for unknown zone {}: skipped.".format(key))
...
def _build_lerp_delta(anim_file, base_file, suffix):
    ...
    """
    sub = _create_node("plusMinusAverage", "head_lerp_sub_" + suffix)
...
def _sum_contributions(plugs, name):
    ...
    if not plugs:
        return None
    pma = _create_node("plusMinusAverage", "head_sum_" + name)
...
def _add_base_and_delta(base_outcolor_plug, delta_plug, name):
    ...
    pma = _create_node("plusMinusAverage", "head_final_" + name)
...
def build_wrinkle_compositor(ai_shader):
    ...
    """
    print("[mh2arnold] Building wrinkle compositor (v3, HLSL ground-truth)...")

    if not cmds.objExists(DX11_SHADER):
        cmds.error("[mh2arnold] " + DX11_SHADER + " not found.")
        return
    ...
    for weight_idx in range(82):
        if weight_idx not in HLSL_WEIGHTS_BY_CHANNEL:
            cmds.warning("[mh2arnold] Weight {} not in HLSL map (?)".format(weight_idx))
            skipped += 1
            continue
        ch_idx, anim_type, anim_map_idx = HLSL_WEIGHTS_BY_CHANNEL[weight_idx]

        # Resolve the mask file & component for this channel
        if ch_idx not in HLSL_CHANNEL_MAP:
            skipped += 1
            continue
        mask_file, comp = HLSL_CHANNEL_MAP[ch_idx]
        if not cmds.objExists(mask_file):
            cmds.warning("[mh2arnold] Mask file '{}' missing — skipping weight {}".format(
                mask_file, weight_idx))
            skipped += 1
            continue
    ...
    print("[mh2arnold]   Built {} color contribs, {} normal contribs ({} skipped).".format(
    ...
    if color_contribs:
        ...
        amp_col = _create_node("multiplyDivide", "head_amp_color")
        ...
        print("[mh2arnold]   baseColor ← base + (Σ color contribs × wrinkleColorIntensity)")

    # === NORMAL: contribs → sum → × intensity → + base → aiNormalMap.input ===
    if normal_contribs:
        normal_sum = _sum_contributions(normal_contribs, "normal")
        amp_nrm = _create_node("multiplyDivide", "head_amp_normal")
        ...
        nmap = "head_aiNormalMap"
        if cmds.objExists(nmap):
            _connect(final_normal, nmap + ".input")
            print("[mh2arnold]   aiNormalMap.input ← base + (Σ normal contribs × wrinkleNormalIntensity)")
        else:
            cmds.warning("[mh2arnold] aiNormalMap node missing — run convert() first.")

    print("[mh2arnold] Wrinkle compositor done.")
    print("[mh2arnold] Debug: set head_aiStandardSurface.wrinkleColorIntensity")
    print("[mh2arnold]        and wrinkleNormalIntensity to 5-10 to amplify wrinkles.")
    print("[mh2arnold] Or call: conv.set_debug_intensity(color=5, normal=5)")
...
def convert(with_wrinkles=True):
    ...
    if not cmds.objExists(DX11_SHADER):
        cmds.error("[mh2arnold] '{}' not found in scene.".format(DX11_SHADER))
        return

    print("[mh2arnold] === STARTING CONVERSION ===")
    _cleanup_previous_run()
    _setup_color_spaces()
    print("[mh2arnold] Color spaces configured.")
    ...
    if sg:
        ...
        print("[mh2arnold] shadingEngine '{}' surfaceShader -> {}".format(sg, ai))
    else:
        cmds.warning("[mh2arnold] No shadingEngine found for {}".format(DX11_SHADER))

    print("[mh2arnold] === DONE ===")
    print("[mh2arnold] Created shader: {}".format(ai))
    if with_wrinkles:
        print("[mh2arnold] Wrinkle deltas: ACTIVE (animated via FRM_WMmultipliers).")
    else:
        print("[mh2arnold] Wrinkle deltas: SKIPPED (with_wrinkles=False).")
    print("[mh2arnold] Test render to validate.")
    return ai
...
def revert_to_dx11():
    ...
    if not cmds.objExists(DX11_SHADER):
        cmds.error("[mh2arnold] DX11 shader '{}' not found.".format(DX11_SHADER))
        return
    sg = _get_shadingengine(DX11_SHADER)
    if not sg:
        ...
        if sgs:
            ...
            cmds.error("[mh2arnold] Cannot locate head shadingEngine.")
            return
    _connect(DX11_SHADER + ".outColor", sg + ".surfaceShader")
    print("[mh2arnold] Reverted: {} -> {}".format(DX11_SHADER, sg))
...
def viewport_debug_on(channel="color"):
    ...
    """
    if channel == "color":
        src_node = "head_amp_color"
    elif channel == "normal":
        src_node = "head_amp_normal"
    else:
        cmds.error("[mh2arnold] channel must be 'color' or 'normal'.")
        return

    if not cmds.objExists(src_node):
        cmds.error("[mh2arnold] '{}' not found — run convert() first.".format(src_node))
        return

    debug_shader = "head_DEBUG_surfaceShader"
    ...
    if not sg:
        cmds.warning("[mh2arnold] No shadingEngine found.")
        return
    _connect(debug_shader + ".outColor", sg + ".surfaceShader")

    print("[mh2arnold] VIEWPORT DEBUG ON ({}) — head shows raw {} delta.".format(channel, channel))
    print("[mh2arnold]   Black = no wrinkle. Bright = zone firing.")
    print("[mh2arnold]   Crank intensity with conv.set_debug_intensity({}=20).".format(channel))
    print("[mh2arnold]   Restore with conv.viewport_debug_off().")
...
def viewport_debug_off():
    ...
    if not cmds.objExists(ARNOLD_SHADER):
        cmds.error("[mh2arnold] {} not found.".format(ARNOLD_SHADER))
        return
    sg = (_get_shadingengine(ARNOLD_SHADER)
          or _get_shadingengine("head_DEBUG_surfaceShader")
          or _get_shadingengine(DX11_SHADER))
    if not sg:
        cmds.warning("[mh2arnold] No shadingEngine found.")
        return
    _connect(ARNOLD_SHADER + ".outColor", sg + ".surfaceShader")
    print("[mh2arnold] VIEWPORT DEBUG OFF — aiStandardSurface restored.")
...
def set_debug_intensity(color=5.0, normal=5.0):
    ...
    """
    if not cmds.objExists(ARNOLD_SHADER):
        cmds.warning("[mh2arnold] {} not found — run convert() first.".format(ARNOLD_SHADER))
        return
    for attr_name, value in (("wrinkleColorIntensity", color),
        ...
        if cmds.attributeQuery(attr_name, node=ARNOLD_SHADER, exists=True):
            ...
            cmds.warning("[mh2arnold] Attribute {} missing — re-run convert().".format(attr_name))
    print("[mh2arnold] Wrinkle intensity set: color={}, normal={}".format(color, normal))
...
def diagnose(zone_filter=None, weight_filter=None):
    ...
    for w_idx in sorted(HLSL_WEIGHTS_BY_CHANNEL.keys()):
        ...
        mask_file, comp = HLSL_CHANNEL_MAP.get(ch_idx, ("?", "?"))
        short_fn = mask_file.replace("maskFile_head_", "")
